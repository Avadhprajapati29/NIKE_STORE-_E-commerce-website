...
const Men = () => {
  ...
  useEffect(() => {
    fetch("/products.json")
      ...
      .then((data) => {
        ...
        setDisplayedProducts(data.menProducts.slice(0, PRODUCTS_PER_LOAD));
      })
      .catch(console.error)
  ...
  }, [displayedProducts, filteredProducts, hasMore, loading]);
  ...
  const loadMoreProducts = () => {
    ...
    if (displayedProducts.length + nextProducts.length >= filteredProducts.length) {
      setHasMore(false);
    }
  };
  ...
  // Scroll animation variant (fade-in)
