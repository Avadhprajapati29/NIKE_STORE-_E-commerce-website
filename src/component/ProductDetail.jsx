...
const ProductDetail = ({ cart, setCart }) => {
    ...
    const { user } = useContext(AuthContext); // ✅ Get user login state
    ...
    useEffect(() => {
        fetch("/products.json")
            ...
            .then(data => {
                ...
                if (foundProduct?.images?.length > 0) setSelectedImage(foundProduct.images[0]);
            })
    ...
    if (!product) return <div className="text-center mt-5 fs-4">Loading product details...</div>;
    ...
    return (
        <div style={{ minHeight: "100vh", position: "relative", overflow: "hidden" }}>
            ...
            <div style={{ backgroundColor: "rgba(0,0,0,0.6)", position: "absolute", inset: 0, zIndex: 1 }}></div>
                    ...
                    <div className="card border-0 shadow-lg rounded-4 p-4" style={{
                        ...
                        <div className="row g-0">
                            <div className="col-md-6 d-flex flex-column align-items-center p-4">
                                <img src={selectedImage || product.img} alt={product.name} className="img-fluid rounded-4" style={{ maxHeight: "450px", objectFit: "contain", transition: "transform 0.3s" }} />
                            ...
                            <div className="col-md-6 p-4 d-flex flex-column justify-content-center">
                                ...
                                <p className="mb-4">{product.desc || "No description available."}</p>
                                    ...
                                    <div className="mb-3">
                                        ...
                                        <div className="d-flex flex-wrap gap-2">
                                            {sizes.map((size, index) => (
                                                <button key={index}
                                                    className={`btn ${selectedSize === size ? "btn-warning text-dark fw-bold" : "btn-outline-light"} rounded-pill px-3`}
                                ...
                                <button className="btn btn-warning w-100 fw-bold py-2 mb-3" onClick={handleAddToCart} style={{ borderRadius: "12px", transition: "0.3s" }}>
                                    🛒 Add to Cart
                                </button>

                                <div className="d-flex justify-content-between mt-2">
                                    <button className="btn btn-outline-light rounded-pill px-4" onClick={() => navigate(from)}>← Back</button>
                                    <Link to="/cart" className="btn btn-outline-warning rounded-pill px-4">🛍 Go to Cart</Link>
                                </div>
                            </div>
                        </div>
                    </div>
